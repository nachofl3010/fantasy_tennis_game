...
exports.actualizarPuntosEquiposRonda64 = functions.runWith({
    memory: '128MB'
  })
      ...
      equiposSnapshot.forEach(async (equipoDoc) => {
        ...
        for (const jugadorRef of jugadorRefs) {
          ...
          if (jugadorDoc.exists) {
            const puntosJugador = jugadorDoc.data().puntos_r64;
            puntuacionEquipo += puntosJugador;
          }
        }

        // Actualiza el campo "puntos_torneo" en el documento del equipo
        await equiposRef.doc(equipoId).update({ puntos_r64: puntuacionEquipo });
      });

      return res.status(200).json({ message: 'Puntos de equipos r_64 actualizados exitosamente.' });
    } catch (error) {
