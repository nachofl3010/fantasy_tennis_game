...
exports.actualizarPuntosTorneoEquipos = functions.runWith({
    memory: '128MB'
  })
      ...
      equiposSnapshot.forEach(async (equipoDoc) => {
        ...
        for (const jugadorRef of jugadorRefs) {
          ...
          if (jugadorDoc.exists) {
            const puntosJugador = jugadorDoc.data().puntos_torneo;
            puntuacionEquipo += puntosJugador;
          }
        }

        // Actualiza el campo "puntos_torneo" en el documento del equipo
        await equiposRef.doc(equipoId).update({ puntos_torneo: puntuacionEquipo });
      });

      return res.status(200).json({ message: 'Puntos de equipos actualizados exitosamente.' });
    } catch (error) {
