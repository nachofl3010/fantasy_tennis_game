...
exports.actualizarPuntosEquiposFinal = functions.runWith({
    memory: '128MB'
  })
      ...
      equiposSnapshot.forEach(async (equipoDoc) => {
        ...
        for (const jugadorRef of jugadorRefs) {
          ...
          if (jugadorDoc.exists) {
            const puntosJugador = jugadorDoc.data().puntos_final;
            puntuacionEquipo += puntosJugador;
          }
        }

        // Actualiza el campo "puntos_torneo" en el documento del equipo
        await equiposRef.doc(equipoId).update({ puntos_final: puntuacionEquipo });
      });

      return res.status(200).json({ message: 'Puntos de equipos final actualizados exitosamente.' });
    } catch (error) {
